  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-6 md:px-8 md:py-10">
        ...
        {/* PROGRESS STATS - show user impact */}
        {jobs.length > 0 && (
          <ProgressStats
            invoicesProcessed={jobs.filter(j => j.type === 'extraction').length}
            accuracyRate={95}
          />
        )}

        {/* UPGRADE PROMPTS - smart conversion triggers */}
        {userProfile && userProfile.docs_this_month >= 8 && userProfile.docs_this_month < 10 && (
          <UpgradePrompt
            currentUsage={userProfile.docs_this_month}
            limit={10}
            mode="banner"
          />
        )}
        {userProfile && userProfile.docs_this_month >= 10 && (
          <UpgradePrompt
            currentUsage={userProfile.docs_this_month}
            limit={10}
            mode="modal"
        ...
        {jobs.some(j => j.status === 'verified') && jobs.filter(j => j.status === 'verified').length === 1 && (
          <NextStepsCard
            title="Great Job!"
