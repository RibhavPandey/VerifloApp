...
import { 
  ...
  FolderOpen, Calendar, ArrowUpRight, Sparkles, MessageSquare
} from 'lucide-react';
...
const Dashboard: React.FC = () => {
  const { jobs, onJobCreated, refreshData, documentsUsed, documentsLimit } = useOutletContext<WorkspaceContextType>();
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-6 md:px-8 md:py-10">
        ...
        {/* PROGRESS STATS */}
        {jobs.length > 0 && (
          <ProgressStats invoicesProcessed={jobs.filter(j => j.type === 'extraction').length} accuracyRate={95} />
        )}

        {/* UPGRADE PROMPTS */}
        {documentsLimit > 0 && documentsUsed >= Math.floor(documentsLimit * 0.8) && documentsUsed < documentsLimit && (
          <UpgradePrompt currentUsage={documentsUsed} limit={documentsLimit} mode="banner" />
        )}
        {documentsLimit > 0 && documentsUsed >= documentsLimit && (
          <UpgradePrompt currentUsage={documentsUsed} limit={documentsLimit} mode="modal" />
        )}

        {/* NEXT STEPS NUDGES */}
        {jobs.length === 1 && jobs[0].type === 'spreadsheet' && (
          <NextStepsCard
            title="Try AI Chat!"
            message="You've uploaded a file. Now ask questions about your data using AI chat."
            actionLabel="Open Chat"
            onAction={() => navigate(`/sheet/${jobs[0].id}`)}
            dismissKey="first-upload-chat"
            variant="info"
        ...
        {needsAttentionJobs.length > 0 && (
          <NextStepsCard
            ...
            message="Review the extracted data and export to Tally, QuickBooks, or CSV."
            actionLabel="Review Now"
            onAction={() => navigate(`/extract/${needsAttentionJobs[0].id}/review`)}
            dismissKey={`extraction-complete-${needsAttentionJobs[0]?.id}`}
            variant="success"
            icon={<CheckCircle2 size={20} />}
          />
        )}
        {jobs.filter(j => j.status === 'verified').length === 1 && (
          <NextStepsCard
            title="Great Job!"
            message="You've completed your first export. Process more invoices to save even more time."
            actionLabel="Process More"
            onAction={() => navigate('/extract/new')}
            dismissKey="first-export-complete"
            variant="success"
